...
class QueryRegularFixedShippingInstruction extends Model {
        ...
        try {
            ...
            foreach ($actual_container as $key => $value) {
                ...
                foreach ($fixedQuantity as $order) {
                    ...
                    $cust_item_no = $order->cust_item_no;
                    $exists = false;
                    ...
                    if (!$exists) {
                        $boxArray[$order_no.$cust_item_no][] = $order->order_no.$order->cust_item_no;
                    }
                }
            }
        ...
        $data->transform(function ($item) use($params){
            if ($item->id_fixed_shipping_instruction_creation) {
                ...
                foreach ($box as $box_item){
                    ...
                    $count_meas_ypmj[] = $box_item['meas_ypmj'] ?? null;
                }
    ...
    public static function printCasemarks($request,$id,$pathToFile,$filename)
    {
        try {
            ...
            $deliv_plan = RegularDeliveryPlan::with('manyFixedQuantityConfirmationBox')->whereIn('id',$id_delivery_plan)->orderBy('urutan','asc')->orderBy('item_no','asc')->get();
            ...
            foreach ($box as $box_item){
                ...
                $count_net_weight += array_sum($box_item['unit_weight_kg']);
                $count_gross_weight += array_sum($box_item['total_gross_weight']);
                $count_meas += in_array($box_item['length'], ['',null]) ? 0 : (($box_item['length'] * $box_item['width'] * $box_item['height']) / 1000000000);
                $gross_weight_per_part[] = $box_item['total_gross_weight'];
            }
