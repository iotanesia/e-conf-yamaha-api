   ...
   public static function detailProduksiBox($params,$id)
    ...
    public static function getGeneratedBox($params,$id_regular_order_entry)
    {
        ...
        $data->transform(function ($item) use ($current_bucket)
        {
            ...
            if(sizeof($labeling) > 0) $is_labeling = 1;
            else $is_labeling = 0;
            return [
        ...
        try {
            ...
            foreach ($request['data'] as $item) {
                ...
                if($current_qty === $item["qty"]){
                    ...
                    $delivery_plan_box = RegularDeliveryPlanBox::where('id_regular_delivery_plan', $item['id'])->get();
                    foreach ($delivery_plan_box as $update) {
                        $update->update([
                            "lot_packing" => $item['lot_packing'],
                            "packing_date" => $item['packing_date'],
        ...
        $data->transform(function ($item) use($params) {
            if ($item->id_shipping_instruction_creation) {
                ...
                $SI->transform(function ($si_item) use ($summary_box) {
                    ...
                    $si_item->summary_box = $si_item->description_of_goods_1;
                ...
                $summary_box_data = RegularDeliveryPlanProspectContainerCreation::select('id_container',
                                                                            ...
                                                                            ->get()->map(function($q){
                                                                                $items = [
                                                                                    ...
                                                                                    'count_container' => $q->count_container
                                                                                ];
                                                                                return $items;
                                                                            });
                ...
                foreach ($summary_box_data as $key => $value) {
                    ...
                    $jml_container[] = $value['count_container'];
                }
