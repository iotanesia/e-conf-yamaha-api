        ...
        <hr>
            ...
            <table style="margin-top: 10px;">
                ...
                @if ($item->datasource == "YPMJ")
                    @foreach ($boxYPMJ as $key => $box_item)
                    ...
                    @foreach ($box[$order['order_no'].$order['cust_item_no']] as $key => $box_item)
                        @for ($i = 0; $i < count($box_item['item_no_series']); $i++)
                            <tr>
                                @if ($key == 0)
                                    @if ($i == 0)
                                        <td class="text-center">
                                            ...
                                            MADE IN INDONESIA <br>
                                            INV. No. {{ $item->no_packaging }} <br>
                                            C/No. : {{ $package_number[$order['order_no'].$order['cust_item_no'].$key] }} - {{ count($box[$order['order_no'].$order['cust_item_no']]) > 1 ? ($package_number[$order['order_no'].$order['cust_item_no'].$key] + count($box[$order['order_no'].$order['cust_item_no']]))-1 : $package_number[$order['order_no'].$order['cust_item_no'].$key] }}
                                        </td>
                                    @else
                                        <td class="no-bt"></td>
                                    @endif
                                ...
                                @endif
                                @if ($i % 2 == 0 && $i == 0)
                                    <td style='padding-bottom:5px;' class='text-center'>{{ $package_number[$order['order_no'].$order['cust_item_no'].$key] }}</td>
                                @else
                                    <td class="no-bt"></td>
                                @endif
                                <td style='padding-bottom:5px;' class='text-center'>{{ $box_item['item_no_series'][$i] }}</td>
                ...
                <tr>
                    ...
                    <td class="text-center">
                        @if ($item->datasource == "YPMJ")
                            {{ count($boxYPMJ) > 1 ? $sum_per_order[$order['order_no'].$order['cust_item_no']]['qty'] : $count_qty }}
                        @else
                            {{ $sum_per_order[$order['order_no'].$order['cust_item_no']]['qty'] }}
                        @endif
                    </td>
                    <td class="text-center">
                        @if ($item->datasource == "YPMJ")
                            {{ count($boxYPMJ) > 1 ? number_format(($sum_per_order[$order['order_no'].$order['cust_item_no']]['nett_weight']),2) : number_format($count_net_weight,2) }}
                        @else
                            {{ number_format(($sum_per_order[$order['order_no'].$order['cust_item_no']]['nett_weight']),2) }}
                        @endif
                    </td>
                    <td class="text-center">
                        @if ($item->datasource == "YPMJ")
                            {{ count($boxYPMJ) > 1 ? number_format(($sum_per_order[$order['order_no'].$order['cust_item_no']]['gross_weight']),2) : number_format($count_gross_weight,2) }}
                        @else
                            {{ number_format(($sum_per_order[$order['order_no'].$order['cust_item_no']]['gross_weight']),2) }}
                        @endif
                    </td>
            ...
            <table style="margin-top: 20px;">
                <tr>
                    ...
                    <td class="no-bo">
                        {{ $item->datasource == "YPMJ" ? (count($boxYPMJ) > 1 ? "Carton Boxes" : "Carton Box") : ($grand_total > 1 ? "Carton Boxes" : "Carton Box") }}
                    </td>
                </tr>
                <tr>
